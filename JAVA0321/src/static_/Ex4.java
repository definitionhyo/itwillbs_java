...
//		sc.instance = null;
		...
		// getInstance() 메서드 호출을 통해 그 값을 가져오기
		...
		if(sc == sc2) {
			System.out.println("sc 와 sc2 는 같은 인스턴스를 참조!");
		} else {
			System.out.println("sc 와 sc2 는 다른 인스턴스를 참조!");
		}
...
//	public SingletonClass() {}
	...
	private SingletonClass() {} // 기본 생성자 
		
	// 2. 자신의 클래스 내에서 인스턴스를 직접 생성 후 참조변수에 저장
	// => 인스턴스 생성없이 외부에서 접근해야하므로 static 키워드 사용
	private static SingletonClass instance = new SingletonClass();
	
	// Getter/Setter 단축키 : Alt + Shift + S -> R
	// 3. 생성된 인스턴스의 주소값을 외부로 리턴하는 Getter 메서드 정의
	// => 인스턴스 생성없이 접근해야 하므로 static 키워드 사용
	// => 인스턴스의 주소값을 저장하는 변수 instance가 private 접근제한자이므로
