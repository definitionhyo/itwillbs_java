...
public class Ex2 {
	...
	public static void main(String[] args) {
		 ...
		 *   => 여러 생성자에서 각각 인스턴스 변수를 중복으로 초기화 하지 않고
		 ...
		 *      (단, 메서드 이름()으로 호출, 생성자는 this() 로 호출하는 차이)
		 *      
		 * < 생성자 this() 호출 기본 문법 >
		 * 생성자 내의 첫번째 라인에서
		 * this([데이터...]);
		...
		// MyDate(int, int) 생성자 호출 => 2023년 3월 1일로 초기화
		MyDate d3 = new MyDate(2023, 3);
		System.out.println(d3.year + "/" + d3.month + "/" + d3.day);
		System.out.println("--------------------------------");
		
		// MyDate(int, int, int) 생성자 호출 => 2023년 3월 20일로 초기화
		MyDate d4 = new MyDate(2023, 3, 20);
...
//		day = 1;
		...
		//    초기화에 사용될 데이터를 전달하여 대신 초기화 수행
		//    = 코드 중복 제거 가능
		
		// 자신의 생성자 내에서 다른 오버로딩 생성자 호출 방법
		// this(전달할 데이터...);
		// 메서드라면 MyDate(데이터..)로 호출하고, 생성자면 this(데이터...)로 호출
...
//		day = 1;
	...
	// 연도(year)와 월(month)를 전달받고, 1일로 초기화하는 생성자 정의
	public MyDate(int year, int month) {
		// MyDate(int, int, int) 생성자를 호출하여
		// 전달받은 연도(year)와 월(month), 1일 값을 전달하여 초기화
		this(year, month, 1);
