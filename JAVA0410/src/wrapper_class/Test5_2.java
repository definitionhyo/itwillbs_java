...
public class Test5_2 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < chArr.length; i++) {
			...
			// => 해당 항목이 포함되어 있을 경우 boolean 타입 변수 true 로 변경
		...
		// 각 항목 포함 여부를 체크하여 포함되었을 경우 점수 1점 증가
		// => 해당 항목값이 true면 변수 +1점씩
		if(isUpperCase) point++;
		if(isLowerCase) point++;
		if(isDigit) point++;
		if(isSpec) point++;
		
		// 점수에 따라 안전도 출력(풀이시 switch ~ case문 사용할 예정!)
		switch(point) {
