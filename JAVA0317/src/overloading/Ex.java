...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *   메서드 이름을 각각 따로 정의하고 않고, 동일한 이름으로
...
//		nm.add(1.0, 3.14); // double, double 타입은 전달 불가!
		...
		// 오버로딩이 적용된 메서드를 정의한 OverloadningMethod 인스턴스 생성
		OverloadingMethod om = new OverloadingMethod();
		
		// 오버로딩 된 메서드 호출 시 파라미터값을 전달하면
		// 오버로딩 메서드 중에서 파라미터가 일치하는 메서드가 자동 호출됨
		// => 단, 메서드 호출 시점에서 각 메서드가 구분되어야 오버로딩 성립됨!
		om.add(10, 20); // add(int, int) 호출됨
		om.add(1.0, 3.14); // add(double, double) 호출됨
//		om.add(1, 3.14);
		om.add(10, 20, 30); // add(int, int, int) 호출됨
	}
