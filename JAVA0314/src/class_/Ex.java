...
//		ani = new Animal();
		...
		// Anmial 클래스로부터 생성된 인스턴스에 접근하여 멤버변수 사용 및 메서드 호출
		// => 인스턴스 접근을 위해서는 반드시 인스턴스 주소가 필요하며,
		//    인스턴스 주소를 담고 있는 참조변수(ani)를 통해 접근이 가능함
		// 1. Animal 인스턴스의 멤버변수 name 과 age 에 값을 저장
//		name = "멍멍이"; // 컴파일 에러 발생! 존재하지 않는 변수이므로 사용 불가
		...
		car.modelName = "그랜져";
		car.cc = 3000;
		System.out.println("제조사 : " + car.companay);
		...
		// Student 클래스 인스턴스 생성
		// - 이름 : 홍길동, 번호 : 1, 과정명 : 핀테크 디지털 금용서비스 자바 개발자
		...
		s1.name = "홍길동";
		s1.id = 1;
		s1.className = "핀테크 디지털 금용서비스 자바 개발자";
		...
		s2.name = "강진석";
		s2.id = 2;
		s2.className = "핀테크 디지털 금용서비스 자바 개발자";
