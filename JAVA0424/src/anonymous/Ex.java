...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *   한번에 하나의 클래스로 상속받는 동시에 인터페이스를 구현하거나 하나 이상의
		 *   인터페이스를 구현할 수 없다.
		...
		np.wake(); // 오버라이딩 된 NormalChild 의 wake() 메서드 호출
...
//		np2.run();
		...
		uac.p.wake();
//		System.out.println(uac.p.childVar); // 접근불가
...
// => 자식 클래스를 명시적으로 선언하여 이미 선언된 자식 클래스로 
//    간단히 객체를 생성하여 사용할 수 있음
// => 재사용성이 높음
// => 그러나, 자식 클래스가 재사용되지 않고, 특정 위치에서 사용할 경우라면
//    자식 클래스를 명시적으로 선언하는 것은 귀찮은 작업이 되며 이럴 경우
//    [익명 자식 객체]를 생성
...
class UseAnonyClass2 {
	public void method() {
		...
		//    따라서 익명객체가 저장된 localP는 메서드가 종료되기 전에 사용해야 함
	} // method() 끝
}
...
class UseAnonyClass3 {
	...
	public void method2() {
		method1(new NormalParent() {
					...
					public void wake() {
						System.out.println(
								"매개변수로 사용된 익명객체의 오버라이딩 된 wake() 메서드");
					}	
		});
	}
