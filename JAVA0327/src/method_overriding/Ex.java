...
public class Ex {
	...
	public static void main(String[] args) {
		...
		// => 슈퍼클래스의 parentPrnt() 메서드는 c 참조변수에서는 보이지 않음
...
class Parent { // class Prarent extends Object
...
class Child extends Parent {
	...
	// 슈퍼클래스 Parent 로부터 상속받은 parentPrn() 메서드 오버라이딩
	// => 슈퍼클래스의 메서드와 이름, 리턴타입, 파라미터가 동일한 메서드 재정의
	// => 슈퍼클래스가 가진 parentPrn() 메서드는 Child 인스턴스에서 보이지 않음!
	public void parentPrn() {
		System.out.println("서브클래스에서 오버라이딩 된 parenPrn()");
	}
...
class Student extends Person {
	...
	// 주의! @Override 어노테이션(Annoation)이 붙어 있을 경우
	// 오버로딩을 수행하거나 오버라이딩 규칙 위반 시 컴파일 에러 발생!
//	@Override
//	public void sleep(int num) { // 컴파일 에러 발생! 오버라이딩 불가!
//		System.out.println("엎드려 잠자기!");
//	}
