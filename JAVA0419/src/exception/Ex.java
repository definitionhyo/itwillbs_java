...
public class Ex {
	...
	public static void main(String[] args) {
		 ...
		 *   => 예외가 발생할 것으로 예상되는 코드를을 try 블록 내에 기술하고,
		 ...
		 * - 컴파일 시점에서 예외 발생 여부를 체크하는 Complie Checked Exception 과
...
//		System.out.println(3 / num);
		// 나눗셈 연산의 피연산자가 0 인 경우 ArithmeticException 예외 발생
...
//		System.out.println(arr[5]);
		...
		// at exception.Ex.main(Ex.java:51)				 	 => 51번 라인에서 예외
...
//		System.out.println(str.length());
		...
		try {
			...
			// ArithmeticExcpetion 예외가 발생했을 경우 처리할 코드를 기술
			// 예외가 발생하지 않을 경우 이 블록은 실행되지 않음
			e.printStackTrace(); // 예외 클래스, 발생 위치, 원인 등 자세히 출력
			...
			System.out.println(
					"배열 인덱스가 잘못 지정됐습니다! - " + e.getMessage());
//		} catch(Exception e) {
