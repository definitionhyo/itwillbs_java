...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 *   익명 객체에 복사해 둔 값이 달라지게 되어 매개변수나 로컬변수를 final로
		 *   선언할 것을 요구함.
		 * - 자바 7이전까지는 final 키워드 없이 선언된 매개변수나 로컬변수를 익명객체에서
		 *   사용 시 컴파일 에러가 발생 했었음.
		 *   (자바 8부터 final 키워드 없이 선언된 매개변수와 로컬변수 사용 시 에러 발생하지 않음)
		 * - final 선언을 하지 않아도 값이 수정될 수 없도록 final의 특성을 부여하기 때문  
		 *  
		 */
