...
//		a = a + 1; // 컴파일에러 발생(byte + int = int 이므로 저장 불가)
		...
		// => 따라서, x1의 값이 5 -> 6 으로 증가한 후 y1에 증가된 값 6이 저장됨
		...
		// => 출력문에 x2 값을 먼저 전달할 후 증감연산자가 사용됨
		
		// 위의 과정에서 6 이 출력된 후 6 -> 7 로 증가되었으므로 7
		...
		// ch + 1 결과값을 출력 (char -> int 로 변환하여 정수를 출력) 하지만,
		// ch 값을 직접 변경하지 않았으므로
		System.out.println(ch); // 변경 전 값인 'A' 출력됨

//		System.out.println(ch++); // A 출력 후 B로 변경
		System.out.println(++ch); // B 변경 후 B가 출력
