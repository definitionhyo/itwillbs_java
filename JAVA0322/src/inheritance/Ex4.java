...
public class Ex4 {
	...
	public static void main(String[] args) {
		 ...
		 *    => Manager4() 생성자 블록 내에 'super();' 가 존재하기 때문
		 ...
		 *    => "Employee4() 생성자 호출됨!" 출력
		 * 6) Manager4 인스턴스가 생성되고, Manager4() 생성자 내의 코드 실행됨
		 *    => "Manager4() 생성자 호출됨!" 출력
		 */
		
		System.out.println("--------------------");
		// Employee4 클래스의 이름, 연봉을 전달받은 생성자
...
//		super(); // 슈퍼클래스 Object 클래스 호출
	...
	// (=> 자바는 파라미터 생성자를 정의하면 기본 생성자를 자동으로 생성하지 않음)
...
class Manager4 extends Employee4 {
	...
	public Manager4() {
		// 서브클래스의 생성자 호출하게 되면
		// 내부적으로 가장 먼저 슈퍼클래스의 기본생성자를 자동으로 출하게됨
		// => 슈퍼클래스 생성자의 작업이 종료된 후 서브클래스 생성자의 작업 수행
//		super(); // 슈퍼클래스의 기본생성자( Employee4() )를 호출
